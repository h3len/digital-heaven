...
function Node(id, pid, name, isleafe , url, title, target, icon, iconOpen, open) {
...
function dTree(objName, iPath, formId) {
	...
	this.config = {
		...
		root				: '../theme/img/base.gif',

		folder			: '../theme/img/folder.gif',

		folderOpen	: '../theme/img/folderopen.gif',

		node				: '../theme/img/page.gif',

		empty				: '../theme/img/empty.gif',

		line				: '../theme/img/line.gif',

		join				: '../theme/img/join.gif',

		joinBottom	: '../theme/img/joinbottom.gif',

		plus				: '../theme/img/plus.gif',

		plusBottom	: '../theme/img/plusbottom.gif',

		minus				: '../theme/img/minus.gif',

		minusBottom	: '../theme/img/minusbottom.gif',

		nlPlus			: '../theme/img/nolines_plus.gif',

		nlMinus			: '../theme/img/nolines_minus.gif'
...
dTree.prototype.add = function(id, pid, name, isleafe ,url, title, target, icon, iconOpen, open) {

	this.aNodes[this.aNodes.length] = new Node(id, pid, name, isleafe , url, title, target, icon, iconOpen, open);
...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.useCheckBox && nodeId!=0){
		...
		//To check if the node isleafe nodes,if it is,get the value of this node,if wanna get more value,write some function by urself;Ex:para_name="'+node.name+'",get out the name atrribute of this node
		//自定input属性，判断节点是否为叶子节点，如果是就取出该节点下的属性，其中isleafe属性为判断其是否为叶子节点，para_name为取node中的name属性
		//alert(node.isleafe);
		str += '<input type="checkbox" name="id"  isleafe="' + node.isleafe + '" id="c'+this.parEnum+'" onClick="caBox(\''+this.formId+'\',\'c'+this.parEnum+'\')" para_name="'+node.name+'" value="' + node.id +'" class="cx"/>';
	}
...
//				}
					/***************************************************
					when allchild node checked,parent node will be checked
						for(j=0;j<regxArray.length;j++){
							var pDiv = regx.substring(0,regx.indexOf(regxArray[regxArray.length-j-1]+"-"));
							var b=true;
							var form1 = document.getElementById(pDiv);
