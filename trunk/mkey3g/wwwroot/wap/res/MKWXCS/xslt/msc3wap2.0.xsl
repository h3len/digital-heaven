          ...
          <xsl:when test="/apps/@isCloud = 'true'">
            <div class="app">
              <form action="{$wapAction}">
				  <!--20101207增加判断输入错误的appid的提示-->
				  <xsl:if test="/apps/@hasApp = 'false'">
					  <font color="#FF0000">无对应的业务系统，请重新输入!</font>
					  <br />
				  </xsl:if>

			   请输入业务系统ID:<input type="text" name="appid" value=""/>
                <br />
...
<!-- 处理bgcolor,background,backgroundalign三个属性 -->
  ...
  <xsl:choose>
    <xsl:when test="/msc/@action !=''">
      <form id="frmPost" name="frmPost" method="post" action="{$wapAction}{/msc/@action}&amp;_MSC_CMD_=QRY">
...
<xsl:template match="a">
  <xsl:choose>
    <xsl:when test ="@realurl">
      <xsl:variable name ="ref">
        <xsl:call-template name ="ReplaceAll">
          <xsl:with-param name ="content" select="@href"/>
          <xsl:with-param name="str">%26</xsl:with-param>
          <xsl:with-param name="replacement">&amp;</xsl:with-param>
        </xsl:call-template>
      </xsl:variable>
      <xsl:variable name ="href">
        <xsl:call-template name ="ReplaceAll">
          <xsl:with-param name ="content" select="$ref"/>
          <xsl:with-param name="str">%3d</xsl:with-param>
          <xsl:with-param name="replacement">=</xsl:with-param>
        </xsl:call-template>
      </xsl:variable>

      <xsl:choose>
        <xsl:when test ="contains($href,'url=')">
          <a href="{substring-after($href,'url=')}" target="_blank">
            <xsl:value-of select="text()" />
          </a>
        </xsl:when>
...
<xsl:template match="img">
 <xsl:choose>
  ...
  <xsl:otherwise>
    <xsl:if test ="@realurl">
    <a href="{substring-after(@href,'url=')}">
        <xsl:apply-templates select="." mode="bll" />
      </a>
    </xsl:if>
    <xsl:if test="not(@realurl)">
      <a>
        <xsl:attribute name="href">
          <xsl:call-template name="handleUrl">
            <xsl:with-param name="url" select="@href" />
      ...
      <a><!-- class="gridHref"-->
        ...
        <xsl:choose>
          <xsl:when test="@icon and @icon != ''">
            <img src="{concat($wapAction,$getimg,@icon)}" class="gridImg_{$gridMode}" >
            </img>
          </xsl:when>
          <xsl:otherwise>
            <img src="{concat($wapAction,$getimg,'res:normal.png')}" class="gridImg_{$gridMode}" />
          </xsl:otherwise>
        </xsl:choose>
...
<xsl:template match="treeitem">
  ...
  <xsl:choose>
    <xsl:when test="@childurl">
      ...
      <img src="{concat($wapAction,$getimg,'res:tree-plus.gif')}" alt="+"/>
      </a>
    </xsl:when>
    <xsl:when test="treeitem">
      <img src="{concat($wapAction,$getimg,'res:tree-minus.gif')}" alt="-"/>
    </xsl:when>
    <xsl:when test="count(following-sibling::*) &gt; 0">
      <img src="{concat($wapAction,$getimg,'res:tree-middle-line.gif')}" alt=""/>
    </xsl:when>
    <xsl:otherwise>
      <img src="{concat($wapAction,$getimg,'res:tree-bottom-line.gif')}" alt=""/>
    </xsl:otherwise>
  </xsl:choose>
  ...
  <xsl:choose>
    <xsl:when test="@icon != ''">
      <img src="{concat($wapAction,$getimg,@icon)}" style="width:14px; height:14px;" alt=""></img>
    </xsl:when>
    <xsl:otherwise>
      <xsl:choose>
        <xsl:when test="@childurl">
          <img src="{concat($wapAction,$getimg,'res:tree-folder.gif')}" alt="+"/>
        </xsl:when>
        <xsl:when test="treeitem">
          <img src="{concat($wapAction,$getimg,'res:tree-folder-open.gif')}" alt="-"/>
        </xsl:when>
        <xsl:otherwise>
          <img src="{concat($wapAction,$getimg,'res:tree-node.gif')}" alt=""/>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:otherwise>
  </xsl:choose>
...
<xsl:template match="login">
   <form id="frmLogin" method="POST" action="{$wapAction}">
	   <br/>用户名：<br/><input name="username" type="text" maxlength="50" tabindex="1" /><br/>
	   <br/>密&#160;&#160;码：<br/><input name="password" type="password" maxlength="20" tabindex="2"/><br/>
...
<!-- fileset template -->
  ...
  <xsl:template match="fileitem" mode="fileset">
    <xsl:choose>
      <xsl:when test="@filetype = 'directory'"><img src="images/folder.png" alt="文件夹"/><a href="{$wapAction}?action=getXml&amp;serverurl={@href}"><xsl:if test="../@enable = 'false'"><xsl:attribute name="href"></xsl:attribute></xsl:if><xsl:value-of select="@caption"/></a>
      </xsl:when>
      <xsl:otherwise>
        <img alt="文件">
  ...
  <xsl:template name="handleUrl">
    ...
    <xsl:if test="$url != ''">
      <xsl:choose>
        ...
        <xsl:when test="starts-with($url,'m.do')"><xsl:value-of select="$url"/></xsl:when>
        <xsl:otherwise>
          <xsl:choose>
            ...
            <xsl:when test="$type = 'file'"><xsl:value-of  select="$wapAction"/>?action=getfile@wap&amp;filename=<xsl:value-of select="concat($appfolder,$url)"/></xsl:when>
          </xsl:choose>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:if>
