    ...
    var GB_ROOT_DIR = "../skin/";
	 ...
	 <script type="text/javascript" src="../scripts/common.js"></script>
		...
		//	var roleid = '<%=request.getParameter("roleid")%>';
			...
			function modrole(){
				if($('rolename').value=='' || $('rolename').value.replace(/\s/g,"")==""){
				 	 DHalert("角色不能为空或空格！");
				 	 return false;
				 }
				 ...
				 if(comment.length>40){
					...
					if(btn=='yes'){
							comment = comment.substring(0,40);	
							 Progress('正在创建角色信息....','角色管理');
							 RoleAction.RoleCreate($('rolename').value,comment,
								{
									callback:function(otext) {
										...
										if(otext=="1"){
											 parent.parent.DHalert("创建角色信息成功");
											 parent.parent.getrolelist();
											 parent.parent.GB_CURRENT.hide()
										}else{
											DHalert("创建角色信息失败，失败原因："+otext);
										}
									},
									errorHandler:function(errorString, exception) {
										CloseProgress();
									    DHalert("创建角色信息失败，失败原因："+errorString);
									}
								});	
							}else{
							return false;	
	...
	<body onkeydown="if(event.keyCode=='13'&&event.srcElement.type!='textarea') return false;">
		...
		<table width="620" border="0" align="center" cellpadding="0"
			...
			<tr>
				...
				<td width="71%">
					<input id="rolename" class="input_text" onmouseover="fEvent('mouseover',this)"onfocus="fEvent('focus',this)"
...
onmouseover="fEvent('mouseover',this)" onmouseout="fEvent('mouseout',this)"></textarea>
		...
		<div class="div_center"><input type="button" class="button_2" value="确认" onclick="modrole()"/> &nbsp;&nbsp;&nbsp;&nbsp;
