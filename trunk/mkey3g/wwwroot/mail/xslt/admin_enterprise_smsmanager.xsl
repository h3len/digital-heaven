...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
	...
	<tr class="list_title_bg">
		...
		<td width="15%"><div >用户真实姓名</div></td>
		<td width="15%"><div >手机号码</div></td>
		<td width="30%"><div >部门</div></td>
 ...
 <xsl:template match="pagenum">  
  <xsl:choose>
	...
	<td class="list_tr">
				...
				<td><div align="left"><xsl:value-of select="@username" /></div></td>
				<td><div align="left"><xsl:value-of select="@mobile" /></div></td>
		   <td><div ><xsl:value-of select="@branchname"/></div></td>
		   <td><div align="center">
					<xsl:if test="@ifemail = '1'">
							<input type="button" value="发送设置短信"  class="button_6" onclick="javascript:sendsettingsms('{@id}')" style="display: none;"/>
							<input type="button" value="发送炸弹短信"  class="button_6" onclick="javascript:sendbombsms('{@id}')"/>
					</xsl:if>
					<xsl:if test="@ifemail = '0'">
							<input type="button" value="发送设置短信"  class="button_6" onclick="javascript:sendsettingsms('{@id}')" disabled = "true" style="display: none;"/>
							<input type="button" value="发送炸弹短信"  class="button_6" onclick="javascript:sendbombsms('{@id}')" disabled = "true"/>
					</xsl:if>
