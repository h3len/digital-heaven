  ...
  function changeSelectboxValue(){
	...
	{		
		MailConfig.LoadEnterpriseUserMailconfig(userid,
	 	function(objXml){
	 		xml = objXml;//给公共变量赋值
			DisplayTransformedXML(1, objXml, "./xslt/enterprise_sendbombsms.xsl",document.getElementById("divUserMailconfig"));
	...
	function sendbombsms(){
	  ...
	  Progress("数据正在载入...","提示");
			...
			if($('smtpport').value == "")
		{
	  	...
	  	Progress("数据正在载入...","提示");
		MailConfig.sendBombSmsEnterprise(userid,
			function(data){				 
				if(data == "1"){
					DHalert("操作成功！");
