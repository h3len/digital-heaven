...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <!--东莞质监-->
  <xsl:variable name="dgzj">http://qts.dg.gov.cn</xsl:variable>
  
  <!-- 公用模板 -->
  <xsl:variable name="amp" select='"&amp;"'/>
  <xsl:variable name ="url">http://oa.zsnet.com</xsl:variable>
  <xsl:variable name="hxq">
    http://<xsl:value-of select="substring-before(substring-after(//url ,'//'),'/')"/>
  </xsl:variable>
  <!-- a标签模板 -->
  ...
  <xsl:template match="img">
    <br/>
    <img src="{$url}/wince/moa.nsf/{@href}" alt="{@title}" />
  </xsl:template>
