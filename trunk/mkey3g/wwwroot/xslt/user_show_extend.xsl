...
<xsl:attribute name="onclick">JavaScript:UserEnableMIPExtend("<xsl:value-of select="@id"/>");</xsl:attribute>
</input>
</td></tr>
</xsl:if>
...
<div style="text-align:right;"><input type="button" id="btnOK" name="btnOK" value=" 返回部门列表 "><xsl:attribute name="onclick">javascript:GetCatalogList("<xsl:value-of select="/root/user/@catalogid" />");GetUserList("<xsl:value-of select="/root/user/@catalogid" />");</xsl:attribute></input></div>
