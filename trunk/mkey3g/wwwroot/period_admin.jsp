...
<title><%= MipWebMoreUi.mip_period_user_1 %></title>
<!-- common.js 通用js调用 -->
...
<script type="text/javascript">
	...
	function UserLogin()
	{
	 ...
	 Progress('<%= MipWebMoreUi.mip_default_user_37 %>','<%= MipWebMoreUi.mip_default_user_38 %>');
	  ...
	  User2.login(eccode,$('username').value,$('password').value,$('rnd').value,4,logintype,logincount,
			{
				callback:function(objXml) {
					if(objXml=="2"){
							CloseProgress();
				  			Progress('<%= MipWebMoreUi.mip_default_user_45 %>','<%= MipWebMoreUi.mip_default_user_46 %>');
						    window.location.href="<%=Constants.periodUrl%>";
				  		}else{
				  			CloseProgress();
...
<!-- Bottom 页面底部 -->
<div id="bottom" class="gray_12"><div style="width:970px;margin:auto;text-align:center;"><%= MipWebMoreUi.mip_default_user_59 %><span class="margin_left_100"><span class="margin_left_100" /><img src="theme/images/bottom_ad1.jpg" /></span></div>
