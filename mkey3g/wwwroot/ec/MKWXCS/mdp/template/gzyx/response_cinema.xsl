...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="a">
    <xsl:choose>
      ...
      <!--电子地图屏蔽-->
      <xsl:when test="contains(@href,'map')">
        <xsl:value-of select="."/>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template match="div">
    <xsl:choose>
      ...
      <xsl:when test="@class='fr pr6 w265 song'"/>
      <xsl:when test="@class='fr pr6 w265 song'"/>
      <!--分页暂时不要-->
      <xsl:when test="@id='pagerDiv'"/>
      <!--屏蔽我的电影院连接-->
      <xsl:when test="@class='mt10'"/>
      <xsl:otherwise>
        <xsl:apply-templates/>
        <br/>
      </xsl:otherwise>
  ...
  <xsl:template match="li" mode="li">
    <span>
      <span width="100%">
        <span align="left">
          <xsl:apply-templates select="div[@class='w245']/a"/>
        </span>
        <span align="right">
