...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template match="/">
        <msc type="form" action="">
            ...
            <body>
              ...
              <!--<a realurl="true" href="http://movie.mtime.com/flash/newvideoplayer.swf?vid=29707&amp;mid=106313">bbbb数据库的话</a>-->
            </body>
        </msc>
    </xsl:template>
    ...
    <xsl:template match="div">
        <xsl:choose>
            ...
            <!---->
            <xsl:when test="@class='ele_inner mt5 pl15 pr15 pb12 __r_c_'"/>
            <xsl:otherwise>
                <xsl:apply-templates/>
                <br/>
            </xsl:otherwise>
    ...
    <xsl:template match="a" mode="fenye">
        <xsl:choose>
            ...
            <xsl:otherwise>
                |<a href="?action=bs-transfer@mdp&amp;url={$movie}/{substring-before(substring-after(//url,'com/'),'/')}/{@href}&amp;nologin=true&amp;dheavenRequestTemplate=gzyx/request_city.xsl" target="_blank">
                    <xsl:value-of select="."/>
                </a>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <!--em标签-->
    <xsl:template match="em[@class='px12 c_a5']">
        <xsl:apply-templates/>
        <br/>
    </xsl:template>
    ...
    <xsl:template match="h3">
        <xsl:choose>
            ...
            <xsl:when test="@class='mt6 px14 lh20' or @class='lh20 px14 mt30' or @class='lh20 px14 mt30' or @class='bold lh20 px14'">
                <br/>
                &#160;
                <br/>
                <strong>
  ...
  <xsl:template match="h3[@class='lh20 px14 mt30']">
    ...
    <!--=======li 标签==============-->
    <xsl:template match="li[@style='list-style: none']"/>

   <!--增加视频播放地址-->
  <xsl:template match ="div[@id='videoPlayer']">
    <xsl:variable name ="href">
      <xsl:call-template name ="ReplaceAll">
        <xsl:with-param name ="content" select="//div[@name='lch']/."/>
        <xsl:with-param name="str">&amp;</xsl:with-param>
        <xsl:with-param name="replacement">%26</xsl:with-param>
      </xsl:call-template>
    </xsl:variable>
    <br/>
    <!--<img src="bofang.jpg" realurl="true" href="{$movie}{$href}"></img>
    <br/>-->
    <a realurl="true" href="{$movie}{$href}">视频播放</a>
  </xsl:template>
</xsl:stylesheet>