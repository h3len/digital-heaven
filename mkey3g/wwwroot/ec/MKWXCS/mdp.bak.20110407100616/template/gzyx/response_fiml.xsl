  ...
  <xsl:template match="/html">
        <msc type="form" islogin="true">
            ...
            <body background="index_bg.png">
                ...
                <xsl:apply-templates select="//div[@id='ciname_out']"/>
            </body>
        </msc>
    ...
    <xsl:template match="a">
        <xsl:choose>
            ...
            <!--屏蔽资料报错-->
            <xsl:when test="contains(text(),'资料报错')">
                <xsl:value-of select="."/>
            </xsl:when>
            <!--电子地图屏蔽-->
    ...
    <xsl:template match="div">
        ...
        <!--过滤影评-->
            ...
            <xsl:when test="@class='clearfix mt12'"/>
            <xsl:when test="@class='ciname_nav clearfix'"/>
            <xsl:when test="@class='ciname_right'"/>
            <xsl:when test="@class='right_col'"/>
            <xsl:when test="@class='mt15 clearfix'"/>
            <xsl:otherwise>
                <xsl:apply-templates/>
                <br/>
            </xsl:otherwise>
    ...
    <xsl:template match="p">
        <xsl:choose>
            ...
            <xsl:when test="@class='song c_fff pb9 pt9'"/>
            <xsl:when test="@class='mb3'"/>
            <xsl:when test="@class='mt9 tr'"/>
            <xsl:otherwise>
            <xsl:apply-templates/>
        <br/>
            </xsl:otherwise>
