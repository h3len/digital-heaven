...
function ProgressBar(){	
	...
	this.divCover.style.cssText = "margin:0px auto;clear:both;text-align:center;position:absolute;background-color:#eeeeee;opacity: 0.1;filter:alpha(opacity=10);"
    ...
    this.divProcesserBar.style.cssText = "z-index:1000;margin:0px auto;text-align:center;position:absolute; width:320px;top:"+(document.body.scrollTop*3+document.body.clientHeight)/3+"px; left:"+(document.body.clientWidth - 320)/2 +"px; height:100;background-color:#f2f2f2;border:1px #84AE5A solid;padding:20px;filter:alpha(opacity=100);";
    this.divProcesserBar.innerHTML = "<span id=\"__processerBarText\" style=\"color:#ff0000;font-weight:bolder;\"></span>";
    this.divProcesserBar.style.display="none";
       
    
    this.divXX = document.createElement("div");
    this.divXX.id = "__divXX";
    this.divXX.style.cssText = "z-index:1000;margin:0px auto;text-align:center; position:absolute; top:"+(document.body.scrollTop*3+document.body.clientHeight)/3+"px; left:"+(document.body.clientWidth-320)/2+"; width:"+(document.body.clientWidth - 320)/2 +"px; height:100;opacity: 0;filter:alpha(opacity=0);";
		 this.divXX.innerHTML = "<iframe width=320 height=120 id=__myiframe style=\"border:0px;background-color:#eee;opacity: 0;filter:alpha(opacity=0);\"/>";
    ...
    document.body.appendChild(this.divProcesserBar);
          ...
          function Show(){
                  ...
                  if(this.isNeedProgressBar)
                          this.divProcesserBar.childNodes.item(0).innerHTML = this.text + "<br/><br/><img src=\"images/loading.gif\"/>";
                  else
                  ...
                  if(this.isNeedConfirmButton){
                        htmlText += "<br/><br/><button onclick=\"javascript:document.body.removeChild(document.getElementById('__divXX'));document.body.removeChild(document.getElementById('__divCover'));document.body.removeChild(document.getElementById('__divProcesserBar'));";
                        if(this.focusItem!="")
                        {
		...
		with (formname){
			...
			for(var i=0; i<len; i++){
				if (elements[i].type=="text"||elements[i].type=="password"||elements[i].tagName=="TEXTAREA"){
					...
					{
						//不能为空，并且判断长度是否大于size
						if ((elements[i].istel=="true") && elements[i].value.length > elements[i].size ){
							_progressBar.isNeedConfirmButton = true;
						    _progressBar.focusItem = elements[i].id;
						    _progressBar.SetStatus("<img src='../images/error.gif' align='absmiddle'> 对不起," + (typeof(elements[i].chname)=="undefined"?"该对象":elements[i].chname) + "格式不正确！");
						    return false;
					...
					{
						//可以为空，但是如果有内容就需要判断
						if (elements[i].value!="" &&  elements[i].value.length > elements[i].size ){
							_progressBar.isNeedConfirmButton = true;
						    _progressBar.focusItem = elements[i].id;
						    _progressBar.SetStatus("<img src='../images/error.gif' align='absmiddle'> 对不起," + (typeof(elements[i].chname)=="undefined"?"该对象":elements[i].chname) + "格式不正确！");
						    return false;
					...
					//长度
					//if (typeof(elements[i].chname)!="undefined" && elements[i].value.length>elements[i].maxlength){
					//_progressBar.isNeedConfirmButton = true;
				    //_progressBar.focusItem = elements[i].id;
				   //_progressBar.SetStatus("<img src='../images/error.gif' align='absmiddle'> 对不起," + (typeof(elements[i].chname)=="undefined"?"该对象":elements[i].chname) + "长度不能超过"+elements[i].maxlength+"个字符！");
				    //return false;
					//}
...
//公共变量
var errorMessage = "系统出现错误或网络超时，请稍候再试！";