...
{//session
	out.print("<script>top.location.href='../default_ecmanager.jsp?url="+request.getServletPath() + "?" +request.getQueryString()+"'</script>");
	return;
}
else
{//权限
 	String uid = (String)request.getSession().getAttribute("user"); 
	String eid = (String)request.getSession().getAttribute("enterpriseid"); 
 	if(!uid.equals(com.dheaven.mip.dataaccess.vo.MipSysuser.Admin_UID)&&!uid.equals(eid)){
...
<script type="text/javascript">
   ...
   //得到企业id
 //var enterpriseid = "<%=request.getSession().getAttribute("enterpriseid")%>";
  ...
  function getuserbyenterpriseid(enterpriseid){
   showtype = 1;
    ...
    MailConfig.getuserbyenterpriseid(enterpriseid,pageid,
			 	function(objXml){
					DisplayTransformedXML(1, objXml, "xslt/admin_enterprise_smsmanager.xsl",document.getElementById("divlist"));
		 			CloseProgress();
				}
			);	
  	}
	...
	//发送设置短信
	function sendsettingsms(userid){
		GB_showCenter("发送设置短信", "../mail/admin_sendsettingsms.jsp?userid="+userid,550,878);
	}
	
	//发送炸弹短信
	function sendbombsms(userid){
		GB_showCenter("发送炸弹短信", "../mail/admin_sendbombsms.jsp?userid="+userid,550,878);
	}
...
<div id="center_nav_box">
	<div id="center_nav">您现在所在位置 >> 移动邮件设置 >> 管理短信</div>
</div>	
<div class="center_list_box">
			<div class="center_list">
			<table width="100%" cellpadding="0" cellspacing="0">
		 	<tr>
				<td colspan="3"  class="gray bold"><span class="margin_left_10">管理短信配置</span></td>	
				<td align="right"><img src="/theme/images/list_search.gif" width="18" height="18" />
				按用户真实姓名查询：<input type="text" id="searchtruename"/><input type="button" value="确定" class="button_2" onclick="javascript:serch('search');"/></td>
		 	</tr>
