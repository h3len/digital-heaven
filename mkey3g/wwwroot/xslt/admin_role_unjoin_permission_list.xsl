...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <table width="100%" class="border" cellpadding="2" cellspacing="0" bgcolor="#ffffff">
      ...
      <!--应用模板开始-->
      <xsl:apply-templates select="/root/permissionlist/permission">
      </xsl:apply-templates>
      <!--应用模板结束-->
  </table>
  </xsl:template>
  ...
  <xsl:template match="permission">
   <tr>
     ...
     <td class="ac b">
      ...
      <td style="padding-left:4px;">
        <a><xsl:attribute name="style">cursor:hand;text-decoration:underline;color:#039;font-weight:bolder;</xsl:attribute><xsl:attribute name="onclick">
          javascript:PermissionShow("<xsl:value-of select="@id"/>");
        </xsl:attribute>
        <xsl:value-of select="@permissionname" />
          <xsl:value-of select="@operate" />
        </a>
    </td>
    <td>
    ...
    <td class="ac">
      <input type="button" class="button" value="加入权限">
        <xsl:attribute name="onclick">
          javascript:JoinPermission("<xsl:value-of select="@id"/>");
        </xsl:attribute>
      </input>
     </td>
   </tr>
