...
<table width="100%" cellpadding="2" cellspacing="0" style="border:1px #ccc solid;" bgcolor="#ffffff">
      <tr class="header">
        ...
        <td width="30%" align="center">企业邮箱配置</td>
      </tr>
      <!--应用模板开始-->
  ...
  <xsl:template match="cat">
    <tr onMouseOut="this.style.backgroundColor='#fff'" onMouseOver="this.style.backgroundColor='#E2EEF3'">
      ...
      <td align="center"><input type="button"><xsl:attribute name="style">cursor:pointer;</xsl:attribute><xsl:attribute name="onclick">JavaScript:currentCatalogId = "<xsl:value-of select="@id"/>";LoadUserMailconfig();</xsl:attribute><xsl:attribute name="value">修改企业邮箱配置</xsl:attribute></input></td>
    </tr>
  </xsl:template>
</xsl:stylesheet>
