...
function deletebranch(branchid){
	...
	function searchuser(){
		 ...
		 User2.searchUserForSel($('searchtype').value,searchinfo,pageid,
				{
					callback:function(objXml) {
			 ...
			 return;
		   	 ...
		   	 if(obj.length>0){
				   	 for(var i=0;i<obj.length;i++){
								   	 	 ...
								   	 	 //检验下有没有重复的
								   	 	 var b=false;
								   	 	 for(var j=0;j<parent.parent.document.getElementById('possess2').length;j++){
								   	 	 	var tmpmobile=parent.parent.document.getElementById('possess2')[j].value;
								   	 	 	tmpmobile=tmpmobile.substring(tmpmobile.lastIndexOf('$')+1);
								   	 	 	if(tmpmobile==mobile||parent.parent.document.getElementById('possess2')[j].value==truename+"$"+truename+"$"+mobile){
								   	 	 		parent.parent.DHalert("<%= MipWebJSPUi.mipweb_user_importcontactusers_46 %>");
								   	 	 		b=true;
								   	 	 		break;
								   	 	 	}
								   	 	 ...
								   	 	 option.text = truename;
								   	 	 option.value = truename+"$"+truename+"$"+mobile;
								   	 	 document.all?parent.parent.document.getElementById('possess2').add(option):parent.parent.document.getElementById('possess2').appendChild(option);
								   	 	 }
		...
		<div style="width:100%">
			...
			<div style="width:100%">
				...
				<div style="width:100%">
					...
					<div style="text-align:center"><input type="button" value="<%= MipWebJSPUi.mipweb_user_importcontactusers_56 %>" onclick="subok()" /></div>
				</div>
			</div>
	</body>
</html>
