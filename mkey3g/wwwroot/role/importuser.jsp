...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function allexit(){
		Ext.Msg.confirm('综合办公提醒您', '您确定要退出？', function(btn){
					if(btn=='yes'){
...
function showuserlist(){
	...
	if(now_branchid==null||now_branchid==''){
	  ...
	  User2.showBranchRoleUserList(now_branchid,pageid,
			{
				callback:function(objXml) {
					//alert(objXml.xml);
				//	$('delmenu').style.display = "";
					CloseProgress();
					DisplayTransformedXML(1, objXml, "../role/xslt/roleuser_list.xsl",document.getElementById("userlist"));
				},
				errorHandler:function(errorString, exception) {
					CloseProgress();
...
function deletebranch(branchid){
	Ext.Msg.confirm('综合办公提醒您', '您确定要删除该部门？', function(btn){
					if(btn=='yes'){
    ...
    if(now_node!=null&&now_node!=undefined){
    	...
    	Ext.Msg.confirm('综合办公提醒您', '您确定要删除该用户？', function(btn){
					if(btn=='yes'){
	...
	function searchuser(){
		 seachtype=2;
		 Progress('正在查询用户.......','登录中'); 
		 User2.searchSysRoleUser($('searchtype').value,$('searchoption').value,pageid,
				{
					callback:function(objXml) {
						CloseProgress();
						 DisplayTransformedXML(1, objXml, "../role/xslt/roleuser_list.xsl",document.getElementById("userlist"));
   ...
   //批量删除用户
    function delUsers()
    {
		...
		Ext.Msg.confirm('综合办公提醒您', '您确定要删除选择的用户？', function(btn){
			if(btn == 'yes')
			{
   ...
   function add(){
	...
	function adduser(){
				...
				RoleAction.JoinUser(roleid,strId,now_branchid,{
							...
							<tr>
								<td background="../theme/images/list_top_bg.jpg">
									<div class="list_top_left">
										<span class="gray_14_bold">角色用户管理</span>
									</div>
