...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
  ...
  <xsl:template match="/error">
    <html xmlns="http://www.w3.org/1999/xhtml" xmlns:wml="http://www.wapforum.org/2001/wml">
      <head>
        <title>
          错误<xsl:value-of select="$wap20Title"/>
        </title>
        ...
        <title>
          <xsl:choose>
            <xsl:when test="/apps/@isCloud = 'true'">
              进入应用<xsl:value-of select="$wap20Title"/>
            </xsl:when>
            <xsl:otherwise>
              应用列表<xsl:value-of select="$wap20Title"/>
            </xsl:otherwise>
          </xsl:choose>
        </title>
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
  ...
  <xsl:template match="/servermsg">
    <html xmlns="http://www.w3.org/1999/xhtml" xmlns:wml="http://www.wapforum.org/2001/wml">
      <head>
        <title>
          提示<xsl:value-of select="$wap20Title"/>
        </title>
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
        <meta http-equiv="cache-control" content="max-age=0" />
        <meta http-equiv="cache-control" content="no-cache" />
  ...
  <xsl:variable name="suffix">
    <xsl:choose>
      <xsl:when test="/msc/@nologin and /msc/@nologin = 'true'">
        <xsl:value-of select="concat('appid=',/msc/@appid,'&amp;nologin=',/msc/@nologin)" />
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="concat('appid=',/msc/@appid)" />
      </xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
  ...
  <xsl:template match="/msc">
    <html xmlns="http://www.w3.org/1999/xhtml" xmlns:wml="http://www.wapforum.org/2001/wml">
      <head>
        <title>
          <xsl:value-of select="/msc/head/title/text()" />
          <xsl:value-of select="$wap20Title"/>
        </title>
        <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
        <meta http-equiv="cache-control" content="max-age=0" />
        <meta http-equiv="cache-control" content="no-cache" />
  ...
  <xsl:template match="item" mode="table">
    <option>
      <xsl:attribute name="wml:onpick">
        <xsl:value-of select="concat($wapAction,@href,'&amp;',$QRY)"/>
      </xsl:attribute>
      ...
      <xsl:when test="attribute::*">
        <div>
          ...
          <xsl:variable name="style">
            <xsl:choose>
              <xsl:when test="@background and @background != ''">
                background-image:url(<xsl:call-template name="handleUrl">
                  <xsl:with-param name="url" select="@background" />
                  <xsl:with-param name="type" select="'image'" />
                </xsl:call-template>);
              </xsl:when>
              <xsl:otherwise>
                <xsl:if test="@bgcolor and starts-with(@bgcolor,'#')">
                  background-color:<xsl:value-of select="@bgcolor" />;
                </xsl:if>
                <xsl:if test="@bgcolor and starts-with(@bgcolor,'0x')">
                  background-color:#<xsl:value-of select="substring-after(@bgcolor,'0x')" />;
                </xsl:if>
              </xsl:otherwise>
            </xsl:choose>
    ...
    <xsl:if test="1 = 1">
      <font class="{@size}">
        ...
        <xsl:if test="@color != '' and starts-with(@color,'0x')">
          <xsl:attribute name="style">
            color:#<xsl:value-of select="substring-after(@color,'0x')"/>
          </xsl:attribute>
        </xsl:if>
        <xsl:apply-templates />
      </font>
    ...
    <hr>
      <xsl:if test="attribute::*">
        <xsl:variable name="style">
          ...
          </xsl:choose><xsl:value-of select="' '"/><xsl:if test="@color and starts-with(@color,'#')">
            <xsl:value-of select="concat(@color,' ')"/>
          </xsl:if><xsl:if test="@color and starts-with(@color,'0x')">
            <xsl:value-of select="concat('#',substring-after(@color,'0x'),' ')"/>
          </xsl:if>solid;
        </xsl:variable>
        <xsl:attribute name="style">
          <xsl:value-of select="$style"/>
    ...
    <xsl:choose>
      <xsl:when test="@childurl">
        <a>
          <xsl:attribute name="href">
            <xsl:call-template name="handleUrl">
              <xsl:with-param name="url" select="@childurl" />
              <xsl:with-param name="type" select="'bs'" />
            </xsl:call-template>&amp;tree_nid=<xsl:value-of select="@nid"/>
          </xsl:attribute>
    ...
    <form id="frmLogin" method="POST" action="{$wapAction}">
      ...
      <input name="username" type="text" maxlength="50" tabindex="1">
        <xsl:if test="/msc/@username != ''">
          <xsl:attribute name="value">
            <xsl:value-of select="/msc/@username" />
          </xsl:attribute>
        </xsl:if>
      </input>
      <br/>
      ...
      <input name="password" type="password" maxlength="20" tabindex="2">
        <xsl:if test="/msc/@password">
          <xsl:attribute name="value">
            <xsl:value-of select="/msc/@password" />
          </xsl:attribute>
        </xsl:if>
      </input>
      <br/>
  ...
  <xsl:template match="fileitem" mode="fileset">
    <xsl:choose>
      <xsl:when test="@filetype = 'directory'">
        ...
        <a href="{$wapAction}?action=getXml&amp;serverurl={@href}&amp;{$suffix}">
          <xsl:if test="../@enable = 'false'">
      ...
      <xsl:otherwise>
        <img alt="文件">
          <xsl:attribute name="src">
            <xsl:value-of select="concat($wapAction,$getimg)"/>
            <xsl:choose>
        ...
        <xsl:when test="starts-with($url,'http')">
          <xsl:choose>
            <xsl:when test="$type = 'bs'">
              <xsl:value-of select="concat($wapAction,$bstransfer,$url,'&amp;',$suffix)"/>&amp;_MSC_CMD_=QRY
            </xsl:when>
            <xsl:when test="$type = 'image'">
              <xsl:value-of select="concat($wapAction,$viewimageremote,$url,'&amp;',$suffix)"/>&amp;_MSC_CMD_=QRY
            </xsl:when>
            <xsl:when test="$type = 'file'">
              <xsl:value-of select="concat($wapAction,$viewfileremote,$url,'&amp;',$suffix)"/>&amp;_MSC_CMD_=QRY
            </xsl:when>
          </xsl:choose>
        </xsl:when>
        <xsl:when test="starts-with($url,'?action=') or starts-with($url,$bstransfer) or starts-with($url,$viewfileremote) or starts-with($url,$viewimageremote) or starts-with($url,$localurl)">
          <xsl:value-of select="$wapAction"/><xsl:value-of select="concat($url,'&amp;',$suffix)"/>&amp;_MSC_CMD_=QRY
        </xsl:when>
        <xsl:when test="starts-with($url,'m.do')">
          <xsl:value-of select="concat($url,'&amp;',$suffix)"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:choose>
            <xsl:when test="$type = 'bs'">
              <xsl:value-of  select="concat($wapAction,$getxml,$url,'&amp;',$suffix)"/>
            </xsl:when>
            <xsl:when test="$type = 'image'">
              <xsl:value-of select="concat($wapAction,$getimg,$url,'&amp;',$suffix)"/>
            </xsl:when>
            <xsl:when test="$type = 'file'">
              <xsl:value-of  select="$wapAction"/>?action=getfile@wap&amp;filename=<xsl:value-of select="concat($appfolder,$url,'&amp;',$suffix)"/>
            </xsl:when>
          </xsl:choose>
        </xsl:otherwise>
      </xsl:choose>
